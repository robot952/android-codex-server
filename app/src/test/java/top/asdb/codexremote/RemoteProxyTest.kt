...
class RemoteProxyTest {
    ...
    fun proxyIsScopedToGeneratedInstallProcess() {
        val script = RemoteBootstrap.installScript("0.144.6", "22.17.0", "http://127.0.0.1:7890")
    ...
    fun proxyRejectsShellWhitespace() {
        RemoteBootstrap.installScript("0.144.6", "22.17.0", "http://127.0.0.1:7890\nevil")
    }

    @Test(expected = IllegalArgumentException::class)
    fun proxyRequiresSupportedScheme() {
        RemoteBootstrap.installScript("0.144.6", "22.17.0", "file:///tmp/proxy")
    }
