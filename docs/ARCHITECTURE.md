...
- 每台服务器行在名称与用户/连接状态之间显示 CPU、内存、根磁盘占用文本；未连接或采样失败显示 `--`，不阻塞连接和会话。
- 资源指标通过独立的只读 SSH exec 采样 `/proc/stat`、`/proc/meminfo` 和 `df -P /`，不安装远端 Agent；仅在服务器/会话列表前台轮询，进入后台暂停。
...
- 顶部 `CODEX` 标题可点击返回服务器列表；当前服务器名称下显示 CPU、内存、根磁盘占用文本。会话行本身不增加资源指标，保持原有高度和信息布局。
