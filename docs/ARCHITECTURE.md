...
| Dart | 3.12.2 |
| App 版本 | 1.8.84+212，来自 flutter_app/pubspec.yaml |
...
- Composer 保持固定底部的一体化边框区域：输入框上方可显示附件，底部顺序固定为加号、更多、权限、上下文圆环、模型/思考强度、
  ...
  viewInsets 与时间线同帧移动，不能让键盘盖住输入框或让正文滞后。顶部更多菜单使用旧版 48 dp 行高和填充图标，固定为紧凑
  `232 dp` 宽；顶部菜单和左下角附件菜单统一使用与对话背景有明确层级差的深灰表面、描边和阴影。Debug 日志项仅在 Debug 模式
...
- 应用版本：`1.8.84+212`。Work 页右上角会话菜单固定为 `232 dp`，不再被较长的 Debug 日志文案撑成
  接近整屏的宽弹层；左下角附件菜单固定为 `184 dp`，保留拍照、上传图片和上传文件的单行布局。
