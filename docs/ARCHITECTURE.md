...
- 资源指标通过独立的只读 SSH exec 采样 `/proc/stat`、`/proc/cpuinfo`、`/proc/meminfo` 和 `df -P /`，不安装远端 Agent；仅在服务器/会话列表前台轮询，进入后台暂停。会话列表中 CPU、内存、硬盘任一指标均可点按，悬浮显示 CPU 占用/核心数、内存总量/已用量和根分区总量/已用量。
...
- 顶部 `CODEX` 标题可点击返回服务器列表；当前服务器名称下显示速度表、内存芯片、存储图标和百分比，点按任一指标可查看完整服务器资源详情。会话行本身不增加资源指标，保持原有高度和信息布局。
