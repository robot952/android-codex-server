...
- Android 应用版本：1.7.15（versionCode 37）
...
- 同一 turn 的相邻子 Agent 活动在时间线收敛为紧凑、独立的 Agent 行。每个 Agent 框内名称右侧
  只显示该 Agent 自己的状态：活动态用固定尺寸旋转指示器，终态显示各自状态文字；不得用组级
...
9. 子 Agent 用稳定身份色的图标表达，状态必须准确且逐个独立显示：名称右侧活动态显示旋转指示器，终态显示各自文字，不显示多余“子agent”标签；点击其 Agent 框或列表项必须能进入独立工作页并安全返回父会话。加载中允许首次返回、重复返回不跳层，提交或审批期间禁止切换会话。
