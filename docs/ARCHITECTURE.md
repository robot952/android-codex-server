...
- 资源指标通过独立的只读 SSH exec 采样 `/proc/stat`、`/proc/meminfo` 和 `df -P /`，不安装远端 Agent；仅在服务器/会话列表前台轮询，进入后台暂停。会话列表中的内存指标可点按，悬浮显示已用内存与总内存。
...
- 顶部 `CODEX` 标题可点击返回服务器列表；当前服务器名称下显示速度表、内存芯片、存储图标和百分比，点按内存指标可查看已用内存与总内存。会话行本身不增加资源指标，保持原有高度和信息布局。
