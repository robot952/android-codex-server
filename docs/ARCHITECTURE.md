...
7. 代码修改通过对应门禁后必须创建 Git 提交，提交信息必须使用中文；提交成功后必须立即将当前分支
   分别推送到 Gitee `origin` 和 GitHub `github`，两个远端都成功才算完成。只有用户明确要求暂缓提交
   或推送时才例外。
...
3. 已保存指纹与服务器当前指纹不同时，显示旧/新 SHA-256 并要求再次确认；确认后替换指纹并自动
   重连，取消则保留旧指纹且不连接。
...
- 长按普通服务器行可拖动排序，拖动中浮起并高亮，接近视口边缘自动滚动；松手后通过既有加密 Profile
  列表保存顺序，重启后恢复。本机 Linux/Windows 面板固定，不参与排序；排序不切换当前服务器或重建连接。
...
- Composer 保持固定底部的一体化边框区域：输入框上方可显示附件，底部顺序固定为加号、更多、权限、上下文圆环、模型/思考强度、
  ...
  viewInsets 与时间线同帧移动，不能让键盘盖住输入框或让正文滞后。顶部更多菜单使用旧版 48 dp 行高和填充图标，宽度在
  `112-232 dp` 范围内按可见菜单项自适应；左下角附件菜单同样在 `112-184 dp` 范围内收缩。两个菜单统一使用与对话背景有明确层级差的深灰表面、描边和阴影。Debug 日志项仅在 Debug 模式
...
| 密码/私钥连接 | 首次显示 SHA-256 指纹；变化时显示旧/新指纹；取消不保存；确认后严格连接 |
...
1. 首页是服务器列表，设置按需展开；默认 SSH 用户为 root。普通服务器支持长按拖动排序并持久保存，
   不影响当前选中项、SSH/Agent 连接或本机运行时面板。
...
确认 host、port、用户名、认证材料和服务器实际 SHA-256；主机/端口变化必须重新探测。已固定的主机
密钥变化时，App 应显示旧/新指纹供用户确认，确认后替换并重连，取消时保留旧指纹；不能静默接受。
检查是否有旧 generation 的连接仍在返回，不要通过自动接受 host key 或无限延长 timeout 掩盖错误。
...
### 17.35 拍照消息回显去重（2026-08-13）

- 应用版本：`1.8.52+179`。图片上传到远端时文件名会增加 UUID 前缀，Codex 回显使用远端 basename，
...
### 17.49 远程图片统一加载进度（2026-08-17）

- 应用版本：`1.8.80+207`。对话图片卡片、用户图片附件和 Markdown 远程图片链接统一经过同一个预览加载
  状态；点击后立即显示不可重复触发的页面级遮罩，不得再出现后台读取图片但界面没有反馈的情况。
- SSH SFTP 和 Windows 本机文件读取都采用流式读取。文件总长度已知时显示下载百分比与已接收/总字节数；
...
### 17.52 Work 弹出菜单宽度与层级（2026-08-19）

- 应用版本：`1.8.85+213`。Work 页右上角会话菜单在 `112-232 dp` 范围内按可见内容自适应，不会因隐藏 Debug 日志项
  继续保留空白宽度；左下角附件菜单在 `112-184 dp` 范围内收缩，保留拍照、上传图片和上传文件的单行布局。
- 两个菜单统一使用 `#303030` 表面、`#505050` 细描边、透明 surface tint 和更明确的阴影，与
...
### 17.55 SSH 指纹变化确认更新（2026-08-26）

- 应用版本：`1.8.93+222`。正式 SSH 连接发现已固定指纹与服务器当前指纹不一致时，不再只显示普通
  连接失败；服务器列表改为弹出不可绕过的安全确认，分别展示已保存和服务器当前的 SHA-256 指纹。
...
### 17.65 服务器长按拖动排序（2026-09-08）

- 应用版本：`1.8.104+234`。服务器列表使用单一滚动视口和 `SliverReorderableList`，长按行浮起后可上下
  拖动，接近视口边缘自动滚动；普通短按连接、设置和断开操作保持原有行为。
- 按服务器 ID 与目标相邻 ID 移动顺序，复用 `StoredProfiles.profiles` 和串行加密保存，不增加存储字段；
  本机 Linux/Windows 条目位置、当前选中服务器、配置内容、会话数据和活动连接保持不变。
- 定向回归覆盖双向长按拖动、重建恢复、普通滚动不排序、边缘自动滚动、无效目标、隐藏本机条目、
