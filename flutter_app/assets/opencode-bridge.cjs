...
const turnsCursorSecret = crypto.randomBytes(32);
...
const sessionApprovalPolicies = new Map();
...
function ensureSyntheticActiveTurn(sessionID, startedAt, approvalPolicy) {
  ...
  const turn = {
    ...
    approvalPolicy: approvalPolicy || sessionApprovalPolicies.get(sessionID) || "untrusted",
    modelReference: "",
  };
  activeTurns.set(sessionID, turn);
  notify("turn/started", {
    threadId: sessionID,
    turn: { id: turnID, status: "inProgress", startedAt: turn.startedAt, items: [] },
  });
  return turn;
}

function normalizeInitialTurnsPageRequest(value) {
  const request = value && typeof value === "object" && !Array.isArray(value)
    ? value
    : {};
  const rawLimit = typeof request.limit === "number" ||
    (typeof request.limit === "string" && request.limit.trim())
    ? Number(request.limit)
    : NaN;
  const limit = Number.isFinite(rawLimit) && Number.isInteger(rawLimit) && rawLimit > 0
    ? Math.min(rawLimit, 100)
    : 4;
  const itemsView = ["full", "summary", "notLoaded"].includes(request.itemsView)
    ? request.itemsView
    : "full";
...
function summaryTurnItem(item) {
  ...
  if (item.type === "userMessage") {
    ...
    return {
      id: item.id,
      type: "userMessage",
      content: content.slice(0, 4).map(function (entry) {
        if (!entry || entry.type !== "text") return null;
        return { type: "text", text: summaryText(entry.text) };
      }).filter(Boolean),
    };
  }
...
function projectInitialTurn(turn, itemsView) {
  if (itemsView === "full") return turn;
  ...
  const projected = Object.assign({}, turn);
  projected.items = (turn && Array.isArray(turn.items) ? turn.items : [])
    .map(summaryTurnItem)
    .filter(Boolean);
  return projected;
}

function turnsCursorError(code, message) {
  const error = new Error(message);
  error.code = code;
  return error;
}

function invalidTurnsCursorError(message) {
  return turnsCursorError(-32602, "Invalid OpenCode turns cursor: " + message);
}

function staleTurnsCursorError(threadID) {
  return turnsCursorError(
    -32011,
    "OpenCode turns cursor is stale for session " + threadID + "; resume the session again",
  );
}

function normalizeTurnsThreadID(value) {
  const threadID = typeof value === "string" ? value.trim() : "";
  if (!threadID || threadID.length > 512) {
    throw invalidTurnsCursorError("threadId is missing or too long");
  }
  return threadID;
}

function turnsThreadFingerprint(threadID) {
  return crypto.createHash("sha256").update(threadID, "utf8").digest("base64url");
}

function turnsRevision(turns) {
  ...
  for (const turn of source) {
    const identity = JSON.stringify([
      ...
      Number(turn && turn.startedAt || 0),
    ]);
    hash.update(String(Buffer.byteLength(identity, "utf8")) + ":");
    hash.update(identity, "utf8");
  }
  return hash.digest("base64url");
}

function encodeTurnsCursor(threadID, revision, offset, sortDirection) {
  const payload = Buffer.from(JSON.stringify({
    v: 1,
    t: turnsThreadFingerprint(threadID),
    r: revision,
    o: offset,
    d: sortDirection,
  }), "utf8").toString("base64url");
  const signature = crypto.createHmac("sha256", turnsCursorSecret)
    .update(payload, "utf8")
    .digest("base64url");
  return payload + "." + signature;
}

function decodeTurnsCursor(value, threadID) {
  const expectedThreadID = normalizeTurnsThreadID(threadID);
  if (typeof value !== "string" || !value || value.length > 512) {
    throw invalidTurnsCursorError("value is missing or too long");
  }
  const segments = value.split(".");
  if (segments.length !== 2 ||
      !/^[A-Za-z0-9_-]+$/.test(segments[0]) ||
      !/^[A-Za-z0-9_-]{43}$/.test(segments[1])) {
    throw invalidTurnsCursorError("value is malformed");
  }
  const expectedSignature = crypto.createHmac("sha256", turnsCursorSecret)
    .update(segments[0], "utf8")
    .digest("base64url");
  const actualBytes = Buffer.from(segments[1], "utf8");
  const expectedBytes = Buffer.from(expectedSignature, "utf8");
  if (actualBytes.length !== expectedBytes.length ||
      !crypto.timingSafeEqual(actualBytes, expectedBytes)) {
    throw invalidTurnsCursorError("signature does not match");
  }
  let cursor;
  try {
    cursor = JSON.parse(Buffer.from(segments[0], "base64url").toString("utf8"));
  } catch (error) {
    throw invalidTurnsCursorError("payload is malformed");
  }
  if (!cursor || typeof cursor !== "object" || Array.isArray(cursor) ||
      cursor.v !== 1 ||
      typeof cursor.r !== "string" || !/^[A-Za-z0-9_-]{43}$/.test(cursor.r) ||
      !Number.isSafeInteger(cursor.o) || cursor.o < 0 ||
      (cursor.d !== "asc" && cursor.d !== "desc")) {
    throw invalidTurnsCursorError("payload fields are invalid");
  }
  if (cursor.t !== turnsThreadFingerprint(expectedThreadID)) {
    throw invalidTurnsCursorError("cursor belongs to another thread");
  }
  return cursor;
}

// Offsets are only trusted while the fixed-size turn identity revision matches. This keeps the
// cursor stateless and prevents a changed session from silently skipping or duplicating turns.
function buildTurnsPage(turns, value, threadID, cursorValue) {
  const request = normalizeInitialTurnsPageRequest(value);
  const normalizedThreadID = normalizeTurnsThreadID(threadID);
  const source = Array.isArray(turns) ? turns : [];
  const revision = turnsRevision(source);
  let offset = 0;
  if (cursorValue !== undefined && cursorValue !== null) {
    const cursor = decodeTurnsCursor(cursorValue, normalizedThreadID);
    if (cursor.d !== request.sortDirection) {
      throw invalidTurnsCursorError("sortDirection does not match");
    }
    if (cursor.r !== revision) {
      throw staleTurnsCursorError(normalizedThreadID);
    }
    offset = cursor.o;
  }
  const ordered = source.slice();
  if (request.sortDirection === "desc") ordered.reverse();
  if (offset > ordered.length) throw staleTurnsCursorError(normalizedThreadID);
  const data = ordered.slice(offset, offset + request.limit).map(function (turn) {
    return projectInitialTurn(turn, request.itemsView);
  });
...
function buildInitialTurnsPage(turns, value, threadID) {
  return buildTurnsPage(turns, value, threadID, null);
}

function buildThreadResumeResponse(thread, value) {
  const responseThread = Object.assign({}, thread);
  const turns = responseThread.turns;
  delete responseThread.turns;
  return {
    thread: responseThread,
    initialTurnsPage: buildInitialTurnsPage(turns, value, responseThread.id),
  };
}

async function hydrateSession(session, statuses, approvalPolicy, messageLimit) {
  const status = statuses && statuses[session.id];
  const busy = isSessionActive(status);
  const messages = await sessionMessages(session.id, messageLimit);
  ...
  if (busy && thread.turns.length && !activeTurn) {
    ...
    const restored = ensureSyntheticActiveTurn(
      session.id,
      latest.startedAt,
      approvalPolicy,
    );
    thread.turns = groupMessages(messages, true, restored.id);
  }
  return thread;
}
...
async function writeGlobalSettings(params) {
  ...
  const effectiveApiKey = apiKey || (
    providerID !== currentProvider ? readStoredApiKey(currentProvider) : ""
  );
  ...
  if (effectiveApiKey) {
    await request("/auth/" + encodeURIComponent(providerID), {
      method: "PUT",
      body: { type: "api", key: effectiveApiKey },
    });
  }
...
function staleTurnError(sessionID, expectedTurnID, activeTurnID) {
  const error = new Error(
    "OpenCode turn is no longer active for session " + sessionID +
    " (expected " + expectedTurnID + ", active " + (activeTurnID || "none") + ")",
  );
  error.code = -32010;
  return error;
}

function validateActiveTurn(params, active, expectedField) {
  const values = params || {};
  const sessionID = typeof values.threadId === "string" ? values.threadId.trim() : "";
  const expectedTurnID = typeof values[expectedField] === "string"
    ? values[expectedField].trim()
    : "";
  if (!sessionID || !expectedTurnID || !active || active.id !== expectedTurnID) {
    throw staleTurnError(sessionID, expectedTurnID, active && active.id);
  }
  return { sessionID: sessionID, turn: active };
}

function requireActiveTurn(params, expectedField) {
  const sessionID = params && typeof params.threadId === "string"
    ? params.threadId.trim()
    : "";
  return validateActiveTurn(
    params,
    sessionID ? activeTurns.get(sessionID) : null,
    expectedField,
  );
}
...
async function handleRequest(method, params) {
  ...
  if (method === "thread/resume" || method === "thread/read") {
    ...
    if (params.approvalPolicy) {
      sessionApprovalPolicies.set(sessionID, params.approvalPolicy);
    }
    const session = await request("/session/" + encodeURIComponent(sessionID));
    const statuses = await statusMap();
    const thread = await hydrateSession(
      session,
      statuses,
      params.approvalPolicy,
      method === "thread/read" ? 200 : undefined,
    );
    if (method === "thread/read") return { thread: thread };
    return buildThreadResumeResponse(thread, params.initialTurnsPage);
  }
  if (method === "thread/turns/list") {
    const sessionID = normalizeTurnsThreadID(params.threadId);
    decodeTurnsCursor(params.cursor, sessionID);
    const values = await Promise.all([
      request("/session/" + encodeURIComponent(sessionID)),
      statusMap(),
    ]);
    const thread = await hydrateSession(values[0], values[1], undefined, undefined);
    return buildTurnsPage(thread.turns, params, sessionID, params.cursor);
  }
  if (method === "turn/start") {
    const turnID = "opencode-turn-" + Date.now().toString(36) + "-" +
      Math.random().toString(36).slice(2, 8);
    sessionApprovalPolicies.set(params.threadId, params.approvalPolicy || "untrusted");
  ...
  if (method === "turn/steer") {
    const active = requireActiveTurn(params, "expectedTurnId");
    startPrompt(active.sessionID, params, active.turn.id);
    return {};
  }
  if (method === "turn/interrupt") {
    const active = requireActiveTurn(params, "turnId");
    await request("/session/" + encodeURIComponent(active.sessionID) + "/abort", {
      method: "POST",
      body: { turnId: active.turn.id },
    });
    return {};
  }
...
async function replyPermissionWithRetry(permission, reply, apiVersion) {
  let lastError;
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      return await replyPermission(permission, reply, apiVersion);
    } catch (error) {
      lastError = error;
      if (attempt < 2) {
  ...
  throw lastError || new Error("OpenCode permission reply failed");
}
...
async function handlePermission(permission, apiVersion) {
  ...
  if (policy === "never") {
    await replyPermissionWithRetry(permission, "always", apiVersion);
    return;
  }
...
async function handleApprovalResponse(message) {
  ...
  try {
    await replyPermissionWithRetry(
      pending.permission,
      accepted ? "once" : "reject",
      pending.apiVersion,
    );
...
async function handleEvent(rawEvent) {
  ...
  if (event.type === "session.status") {
    ...
    if (sessionID && isSessionActive(properties.status) && !activeTurns.has(sessionID)) {
      ensureSyntheticActiveTurn(sessionID, Date.now());
    }
...
async function reconcileActiveSessions() {
  ...
  const sessions = Array.isArray(values[0]) ? values[0] : [];
  const statuses = values[1] || {};
  const known = new Set(sessions.map(function (session) { return session.id; }));
  for (const session of sessions) {
    const status = statuses[session.id];
    if (isSessionActive(status)) {
      ensureSyntheticActiveTurn(session.id, Date.now());
    } else if (statusType(status) === "idle" && activeTurns.has(session.id)) {
      await completeSession(session.id, null);
    }
  }
  for (const sessionID of Array.from(activeTurns.keys())) {
    if (!known.has(sessionID)) activeTurns.delete(sessionID);
  }
}
...
module.exports = {
  ...
  normalizeInitialTurnsPageRequest: normalizeInitialTurnsPageRequest,
  ...
  staleTurnErrorCode: -32010,
  staleTurnError: staleTurnError,
  validateActiveTurn: validateActiveTurn,
  invalidTurnsCursorErrorCode: -32602,
  staleTurnsCursorErrorCode: -32011,
  decodeTurnsCursor: decodeTurnsCursor,
  turnsRevision: turnsRevision,
  buildTurnsPage: buildTurnsPage,
  buildInitialTurnsPage: buildInitialTurnsPage,
  buildThreadResumeResponse: buildThreadResumeResponse,
};
