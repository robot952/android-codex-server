...
# install-codex-pinned.sh is deliberately kept for direct stdio mode and is not accepted by the
# daemon command, so fail before the CLI creates any daemon state. Note that a successful daemon
# bootstrap also launches Codex's detached updater; it is not a strict fixed-version mode.
...
version check. Use the repository's pinned npm direct mode for a durable version pin:
...
if [[ ! -x "$STANDALONE_BIN" ]]; then
    cat >&2 <<EOF
Durable Codex daemon mode is unavailable for this installation.

The repository installer uses npm and supports direct mode only:
...
Use direct stdio mode when a strict fixed Codex version is required.
EOF
