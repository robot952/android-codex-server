...
4. 代码修改完成并通过任务风险对应的测试后，必须创建 Git 提交；提交信息必须使用中文，提交成功后
   必须立即将当前分支分别推送到 Gitee `origin` 和 GitHub `github`，两个远端均推送成功才算完成，
   不得只依赖当前分支的 upstream。只有用户明确要求暂缓提交或推送时才例外。每次向用户交付 APK
   或下载链接时，无需用户再次提醒，最终答复必须同时给出完整的内网和外网下载地址，并完成构建、验签
   和下载回验。
...
`./scripts/dev-workflow.sh <mode> --force`。本地发布只执行 `publish`，未获用户明确允许不得触发
`release`；提交完成后按强制规则将当前分支同时推送到 `origin` 和 `github`。
